    ...
    public void init() {
        AnimalShelter cats = new AnimalShelter(
                "Приют для кошек в нашем любимом городе." +
                        "В настоящий момент здесь содержится 159 кошек. За более чем два года нами проделана " +
                ...
                "Нужна прививка от бешенства!",
                "Дебилам животных не даем!"
        );
        AnimalShelter dogs = new AnimalShelter(
                "Приют для собак в нашем любимом городе." +
                        "В настоящий момент здесь содержится 5 собак. " +
                        "Обращаем Ваше внимание, что приют-это благотворительная некоммерческая организация, " +
                        "спонсоров и финансирования у нас нет, а потому мы полностью зависим от пожертвований граждан.",
                AnimalType.DOG,
                "+ 7 999 999 99 99, +7 888 888 88 88",
        ...
        Endpoint reportInfo_CAT = new Endpoint(
                "/reportInfo_CAT",
                "Отправить отчет",
                "В данном разделе вы можете отправить отчет по вверенному вам котику.",
                animalShelterRepository.findById(1L).orElseThrow(),
                mainMenu_cat
        );
...
// ---  этап 2
        ...
        Endpoint documents_CAT = new Endpoint(
                "/documents_CAT",
                "Документы",
                "Чтобы мы могли отдать вам животное вам нужно подготовить справку об отсутствии аллергии на кошек у всех членов вашей семьи и знакомы. Так же от них нужна справка от нарколога и психиатра о том, что они не состоят на учете. Рекомендуем пройти военно-летную комиссию, уход за котиком это большая ответственность!",
                animalShelterRepository.findById(1L).orElseThrow(),
                howToMenu_CAT
        );
        ...
        Endpoint adult_CAT = new Endpoint(
                "/adult_CAT",
                "Дом для котика",
                "Взрослому котику необходима большая площадь, рассчитывайте примерно 30 кв.м. на каждого члена семьи + 50 кв.м. для одного котика.",
                animalShelterRepository.findById(1L).orElseThrow(),
                howToMenu_CAT
        );
        ...
        Endpoint reasonOfRefusal_CAT = new Endpoint(
                "/reasonOfRefusal_CAT",
                "Причины отказа",
                "Не выдаем животных дебилам, наркоманам, аллергикам.",
                animalShelterRepository.findById(1L).orElseThrow(),
                howToMenu_CAT
        );
...
// --- этап 2
        ...
        Endpoint reportInfo_DOG = new Endpoint(
                "/reportInfo_DOG",
                "Отправить отчет",
                "В данном разделе вы можете отправить отчет по вверенной вам собаке.",
                animalShelterRepository.findById(2L).orElseThrow(),
                mainMenu_DOG
        );
...
// ---  этап 2
        ...
        Endpoint documents_DOG = new Endpoint(
                "/documents_DOG",
                "Документы",
                "Чтобы мы могли отдать вам СОБАКЕНА вам нужно подготовить справку об отсутствии аллергии на СОБАКЕНОВ у всех членов вашей семьи и знакомых. Так же от них нужна справка от нарколога и психиатра о том, что они не состоят на учете. Рекомендуем пройти военно-летную комиссию, уход за котиком это большая ответственность!",
                animalShelterRepository.findById(2L).orElseThrow(),
                howToMenu_DOG
        );
        ...
        Endpoint adult_DOG = new Endpoint(
                "/adult_DOG",
                "Дом для собаки",
                "Взрослой собаке необходима площадь, немногим больше, чем для щенка. Рассчитывайте примерно 10 кв.м. на каждого члена семьи и + 0,7 кв.м. для одной собаки.",
                animalShelterRepository.findById(2L).orElseThrow(),
                howToMenu_DOG
        );
